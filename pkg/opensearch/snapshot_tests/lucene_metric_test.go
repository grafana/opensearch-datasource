...
func Test_metric_max_response_fluffles(t *testing.T) {
	responseFromOpenSearch, err := os.ReadFile("testdata/lucene_metric_max.response_from_opensearch.json")
	require.NoError(t, err)
	queries, err := setUpDataQueriesFromFileWithFixedTimeRange(t, "testdata/lucene_metric_max.query_input.json")
	require.NoError(t, err)
	...
	responseForRefIdA, ok := result.Responses["A"]
	assert.True(t, ok)
	experimental.CheckGoldenJSONResponse(t, "testdata", "lucene_metric_max.comparison_with_frontend.golden", &responseForRefIdA, false)
}
...
func Test_metric_sum_response_fluffles(t *testing.T) {
	responseFromOpenSearch, err := os.ReadFile("testdata/lucene_metric_sum_group_by_date_histogram.response_from_opensearch_2.json")
	require.NoError(t, err)
	queries, err := setUpDataQueriesFromFileWithFixedTimeRange(t, "testdata/lucene_metric_sum_group_by_date_histogram.query_input_2.json")
	require.NoError(t, err)
	openSearchDatasource := opensearch.OpenSearchDatasource{
		HttpClient: &http.Client{
			Transport: &queryDataTestRoundTripper{body: responseFromOpenSearch, statusCode: 200, requestCallback: func(req *http.Request) error { return nil }},
	...
	responseForRefIdA, ok := result.Responses["A"]
	assert.True(t, ok)
	experimental.CheckGoldenJSONResponse(t, "testdata", "lucene_metric_sum_group_by_date_histogram.comparison_with_frontend.golden", &responseForRefIdA, false)
}
