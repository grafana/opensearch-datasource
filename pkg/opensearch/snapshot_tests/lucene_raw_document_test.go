...
func Test_raw_document_request_no_time_range(t *testing.T) {
	queries, err := setUpDataQueriesFromFileWithFixedTimeRange(t, "testdata/lucene_raw_document_no_time_range.query_input.json")
	require.NoError(t, err)
	var interceptedRequest []byte
	openSearchDatasource := opensearch.OpenSearchDatasource{
		HttpClient: &http.Client{
	...
	expectedRequest := `{"ignore_unavailable":true,"index":"","search_type":"query_then_fetch"}
{"fields":[{"field":"timestamp","format":"strict_date_optional_time_nanos"}],"query":{"bool":{"filter":{"query_string":{"analyze_wildcard":true,"query":"FlightNum:*M"}}}},"size":480}
`
	assert.Equal(t, expectedRequest, string(interceptedRequest))
}
