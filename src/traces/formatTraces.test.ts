      ...
      expect(dataFrame).toMatchObject({
        ...
        fields: [
          ...
          },
          {
            name: 'Last Updated',
            type: 'time',
            values: ['yesterday', '2 days before yesterday'],
            config: {},
          },
        ],
      });
  ...
  describe('createTraceDataFrame', () => {
    it('should return in the data frame the fields needed for trace view', () => {
      ...
      expect(singleDataFrame.fields.find((field) => field.name === 'traceID')?.values).toEqual([
        '0000000000000000213ce26adf2b30d0',
        '0000000000000000213ce26adf2b30d0',
      ]);
      ...
      expect(singleDataFrame.fields.find((field) => field.name === 'serviceName')?.values).toEqual(['driver', 'mysql']);
